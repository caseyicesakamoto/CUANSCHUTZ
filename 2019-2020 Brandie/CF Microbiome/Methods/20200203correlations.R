...
# cult3 <- read_excel("C:/Users/Casey/Desktop/WorkData/Microbiome Brandie 2019-2020/cult3.xlsx") # desk
# 
...
# write.csv(patient_df, "patientdf.csv")
#####---------------#######

# desktop
library(tidyverse)
library(gridExtra)
patient_df <- read.csv("C:/Users/Casey/Desktop/WorkData/Microbiome Brandie 2019-2020/patient_df.csv")
###### Change in FEV vs change in MH #####
FEV_BD_cor = ggplot(data = patient_df) + 
  geom_point(aes(MH_BD,change_fev_perc_v1v2)) + theme_classic() + 
  coord_cartesian(xlim = c(0.45,1)) + labs(title = "A",
                                           x = "Beta-Diversity", y = "%-Change in FEV1")
...
FEV_cfu_cor = ggplot(data = patient_df) + 
  ...
  labs(title = "C",
       x = "Change in Log CFU", y = "%-Change in FEV1")
# 3 missing a v2
# corr = -0.232
...
####### save plots
grid.arrange(FEV_BD_cor,FEV_load_cor,FEV_cfu_cor,ncol = 3)
