...
# weight
summary(aov(latent_class~X_zwei,data = BONUS_al3)) # significant 
# gender
chisq.test(table(BONUS_al3$latent_class, BONUS_al3$sex_std)) # sig
# genotype
chisq.test(table(BONUS_al3$latent_class, BONUS_al3$class)) # sig
# mic_il
chisq.test(table(BONUS_al3$latent_class, BONUS_al3$mic_il)) # sig
# age first abx
summary(aov(latent_class~age_firstabx,data = BONUS_al3)) # significant
# feed
chisq.test(table(BONUS_al3$latent_class, BONUS_al3$feed)) # sig
# abx status
chisq.test(table(BONUS_al3$latent_class, BONUS_al3$curr_on)) # sig
# number abx on
summary(aov(latent_class~on,data = BONUS_al3)) # not significant
# cumulative abx days
summary(aov(latent_class~cum,data = BONUS_al3)) # not significant

#looks like important ones are gender, weight, genotype, meconium ileus, age first abx, feed, abx status
...
cov_init_mod = lcmm(Streptococcus ~ Age + factor(class) + factor(sex_std) + factor(mic_il) + age_firstabx + curr_on +  feed + X_zwei,
                    random = ~ Age, ng = 1, link = "5-quant-splines", subject = "patient_id", data = BONUS_al3)
# runs with class
model_aim2 = gridsearch(lcmm(Streptococcus ~ Age + factor(class) + factor(sex_std) + factor(mic_il) + age_firstabx + curr_on +  feed + X_zwei,
                             random = ~ Age, ng = 4, mixture = ~Age, link = "5-quant-splines", subject = "patient_id", data = BONUS_al3),
